...
from six2one.storage.models import JobKind, QueueJob, SourceRunId
...
class Queue:
    ...
    def enqueue(self, kind: JobKind, payload: Mapping[str, Any], *, source_run_id: SourceRunId | None = None, priority: int = 0, max_attempts: int | None = None, metadata: Mapping[str, Any] | None = None, available_at: datetime | None = None) -> QueueJob:
        ...
        job_id = self.store.queue.enqueue(
            kind,
            validated,
            source_run_id=source_run_id,
            priority=priority,
            max_attempts=max_attempts or job.max_attempts,
            available_ms=int(available_at.timestamp() * 1000) if available_at is not None else None,
        )
        return self.store.queue.get(job_id)
